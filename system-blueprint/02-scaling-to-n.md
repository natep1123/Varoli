...
The human language application document (`./application-theory/human-language/00-overview-HL.md`) asks what happens when the architecture is instantiated as language. The scaling-to-N test asks the more basic question: is language one local expression of a generally scalable architecture, or is language the natural limit of the architecture?

The cross-domain translation document (`./application-theory/cross-domain-translation/00-overview-CDT.md`) assumes that multiple systems can be related through shared architectural properties. The scaling-to-N test asks whether the architecture remains structurally sufficient as those systems proliferate.
